...
angle_ref_ramp[int(7/dt_ctrl_angle)+1:int(10/dt_ctrl_angle),1] = np.linspace(0,P*math.pi/180,
   angle_ref_ramp[int(7/dt_ctrl_angle)+1:int(10/dt_ctrl_angle),1].size, endpoint=True)
...
##########################################################
ts = time.time()
name = "RampResponse_Angle"+datetime.datetime.fromtimestamp(ts).strftime("_%Y%m%d%H%M%S")
...
for t in np.arange(dt_sim,T_sim+dt_sim,dt_sim):
    ...
    if abs(t/dt_ctrl_angle - round(t/dt_ctrl_angle)) < 0.000001 :
        ...
        # perfect measurement
        angle_ref = angle_ref_ramp[k,:]; k += 1 
