...
dt_ctrl_pos_v = 0.02  # 50 Hz
pid_vx = pid.PID(4, 0, 0, 20, -20, 0.01)
pid_vy = pid.PID(4, 0, 0, 20, -20, 0.01)
pid_vz = pid.PID(5, 0, 0, 20, -20, 0.01)

dt_ctrl_pos_p = 0.02  # 50 Hz
pid_x = pid.PID(1, 0, 0, 20, -20, 0.01)
pid_y = pid.PID(1, 0, 0, 20, -20, 0.01)
pid_z = pid.PID(4, 0, 0, 10, -10, 0.01)
...
##########################################################
pos_ref_step = np.zeros([int(T_sim/dt_ctrl_pos_p)+1,4])
pos_ref_step[:,2] = 3.0

pos_ref_step[int(3/dt_ctrl_pos_p)+1:int(13/dt_ctrl_pos_p),0] = 20.0
pos_ref_step[int(3/dt_ctrl_pos_p)+1:int(13/dt_ctrl_pos_p),3] = 120*math.pi/180

pos_ref_step[int(23/dt_ctrl_pos_p)+1:int(33/dt_ctrl_pos_p),1] = 20.0
pos_ref_step[int(23/dt_ctrl_pos_p)+1:int(33/dt_ctrl_pos_p),3] = 120*math.pi/180

pos_ref_step[int(43/dt_ctrl_pos_p)+1:int(53/dt_ctrl_pos_p),2] = 20.0
pos_ref_step[int(43/dt_ctrl_pos_p)+1:int(53/dt_ctrl_pos_p),3] = 120*math.pi/180

pos_ref_step[int(63/dt_ctrl_pos_p)+1:int(73/dt_ctrl_pos_p),0] = -20.0
pos_ref_step[int(63/dt_ctrl_pos_p)+1:int(73/dt_ctrl_pos_p),1] = -20.0
pos_ref_step[int(63/dt_ctrl_pos_p)+1:int(73/dt_ctrl_pos_p),3] = 120*math.pi/180
...
##########################################################
ts = time.time()
name = "StepResponse_PosCtrl_03"+datetime.datetime.fromtimestamp(ts).strftime("_%Y%m%d%H%M%S")
...
for t in np.arange(dt_sim,T_sim+dt_sim,dt_sim):
    ...
    #------------------------------------begin controller --------------------------------------------
    if abs(t/dt_ctrl_pos_p - round(t/dt_ctrl_pos_p)) < 0.000001 :
        ...
        vx_ref = pid_x.run(ref[0] - meas_pos[0], dt_ctrl_pos_p)
        vy_ref = pid_y.run(ref[1] - meas_pos[1], dt_ctrl_pos_p)
        vz_ref = pid_z.run(ref[2] - meas_pos[2], dt_ctrl_pos_p)
    
    if abs(t/dt_ctrl_pos_v - round(t/dt_ctrl_pos_v)) < 0.000001 :
        
        meas_yaw = qrb.rpy[2]
        
        T1 = pid_vx.run(vx_ref-meas_ve[0],dt_ctrl_pos_v)
        T2 = pid_vy.run(vy_ref-meas_ve[1],dt_ctrl_pos_v)
        R = np.array([[math.sin(meas_yaw), -math.cos(meas_yaw)],
                              [math.cos(meas_yaw), math.sin(meas_yaw)]])
        angle_ref[0:2] = 1/envir.g *R@np.array([T1,T2])
        ...
        T3 = pid_vz.run(vz_ref - meas_ve[2], dt_ctrl_pos_v)
        thrust_ref = qrb.mass*envir.g + qrb.mass*T3 
