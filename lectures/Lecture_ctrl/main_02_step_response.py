...
##########################################################
angle_ref_step = np.zeros([int(T_sim/dt_ctrl_angle)+1,3])
...
#angle_ref_step[int(3/dt_ctrl_angle)+1:int(6/dt_ctrl_angle),1] = -80*math.pi/180 # rad/sec
#angle_ref_step[int(6/dt_ctrl_angle)+1:int(10/dt_ctrl_angle),1] = 80*math.pi/180 # rad/sec

#angle_ref_step[int(3/dt_ctrl_angle)+1:int(6/dt_ctrl_angle),2] = -179*math.pi/180 # rad/sec
#angle_ref_step[int(6/dt_ctrl_angle)+1:int(10/dt_ctrl_angle),2] = 179*math.pi/180 # rad/sec
...
##########################################################
ts = time.time()
name = "StepResponse_Angle"+datetime.datetime.fromtimestamp(ts).strftime("_%Y%m%d%H%M%S")
...
for t in np.arange(dt_sim,T_sim+dt_sim,dt_sim):
    ...
    if abs(t/dt_ctrl_angle - round(t/dt_ctrl_angle)) < 0.000001 :
        ...
        # perfect measurement
        angle_ref = angle_ref_step[k,:]; k += 1 
